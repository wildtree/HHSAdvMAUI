...
namespace HHSAdvMAUI
{
    public class ZObjects
    {
        ...
        {
            using (var fs = File.OpenRead(Path.Combine(ZSystem.DataFolder, fileName)))
            {
                fs.Seek((Id + 1) * blockSize, SeekOrigin.Begin);
                byte[] b = new byte[blockSize];
                using (var br = new BinaryReader(fs))
                {
                    b = br.ReadBytes(b.Length);
                }
                obj = (Id == 14) ? new ZTeacher(b) : new ZObject(b);
            }
        }
        public int Id
        {
        ...
        private class ZObject
        {
            ...
            {
                int o = offset;
                SKColor b = cv.GetPaletteColor(buf[o++]);
                if (draft) b = new SKColor(0xffc0c000); //new SDL.SDL_Color { r = 192, g = 192, b = 0, a = 255 };
                int xs = buf[o++] / 2;
                int ys = buf[o++];
                ...
                {
                    SKColor c = cv.GetPaletteColor(buf[o++]);
                    if (draft) c = b;
            ...
            private int OutLine(Canvas cv, int offset,SKColor col, int ox, int oy)
            {
                int x0, y0, x1, y1;
